      ...
      <ul className="menu menu-horizontal">
        {items.map((item) => {
          return (
            <li
              key={item.href}
              className={pathName === item.href ? "gradient-text" : ""}
            >
...
const Drawer = ({ items, pathName }) => {
  return (
    <div
      className="drawer-side h-auto max-h-fit"
...
const DisplayModeToggle = ({ toggleDisplayMode }) => {
  const handleToggleClick = (event) => {
    event.preventDefault();
    toggleDisplayMode();
  };
  return (
    <label className="swap swap-rotate" onClick={handleToggleClick}>
      <input type="checkbox" />
      <svg
  ...
  return (
    ...
    <nav className={className}>
      <input
        id="my-drawer-3"
        type="checkbox"
        className="drawer-toggle rounded-lg"
      />
      ...
      <Drawer items={menuItems} pathName={pathName} />
    </nav>
  );
}
